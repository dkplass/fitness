...
interface IExerciseCardProps {
  exercise: IWorkoutOfDay;
  index: number;
}

export default function ExerciseCard(props: IExerciseCardProps) {
  ...
  return (
    <div className="p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap">
      ...
      <div className="flex flex-col bg-slate-950 rounded gap-2 p-2">
        {exercise.description.split("____").map((val) => {
        ...
        {["reps", "rest", "tempo"].map((info, index) => {
          return (
            ...
            >
              ...
              {exercise[info as keyof IWorkoutOfDay] && (
                <p className="font-medium">
                  {exercise[info as keyof IWorkoutOfDay] as []}
                </p>
              )}
            </div>
          );
